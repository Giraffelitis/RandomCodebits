...
bool GameplayState::Update(bool processInput)
{
	...
	if (m_beatLevel)
	{
		...
		if (m_skipFrameCount > kFramesToSkip)
		{
			...
			{
				Utility::WrtieHighScore(m_player.GetMoney());
...
void GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		case ActorType::WaterJug:
		{
			WaterJug* collidedWaterJug = dynamic_cast<WaterJug*>(collidedActor);
			assert(collidedWaterJug);
			if (!m_player.HasWater())
			{
				m_player.PickupWater();
				collidedWaterJug->Remove();
				m_player.SetPosition(newPlayerX, newPlayerY);
				AudioManager::GetInstance()->PlayWaterPickupSound();
			}
		...
		case ActorType::Lava:
		{
			...
			assert(collidedLava);
			if (!collidedLava->IsDisabled())
			{
				if (m_player.HasWater())
				{
					collidedLava->Disable();
					m_player.UseWater();
					m_player.SetPosition(newPlayerX, newPlayerY); \
						break;
				}
				...
				{
					AudioManager::GetInstance()->PlayLavaSound();
					m_player.KillPlayer();
					AudioManager::GetInstance()->PlayLoseSound();
					m_pOwner->LoadScene(StateMachineExampleGame::SceneName::Lose);
					break;
		...
		case ActorType::Teleporter:
		{
			...
			for (int i = 0; i < (int)m_pLevel->m_pActors.size(); i++)
			{
				auto actor = m_pLevel->m_pActors[i];

				if (actor->GetType() == ActorType::Teleporter && actor->GetColor() == collidedActor->GetColor())
				{
					if (actor->GetXPosition() != collidedActor->GetXPosition() || actor->GetYPosition() != collidedActor->GetYPosition())
					{
						newPlayerX = actor->GetXPosition();
						newPlayerY = actor->GetYPosition();
						break;
					}
				}
			}
...
void GameplayState::Draw()
{
	...
	actorCursorPosition.X = (short int)m_player.GetXPosition();
	actorCursorPosition.Y = (short int)m_player.GetYPosition();
	...
	currentCursorPosition.Y = (short int)m_pLevel->GetHeight();
...
void GameplayState::DrawHUD(const HANDLE& console)
{
	...
	cout << "WASD = Movement " << Level::WAL << " Z = Drop Key " << Level::WAL;
